...
The finished jar is written to `build/libs/bm3-plasma-1.0.0.jar`. A prebuilt jar is also included in `release/` (with its SHA-256 checksum) if you don't want to build.

## Installation

1. Copy `build/libs/bm3-plasma-1.0.0.jar` (or the one in `release/`) into the `mods/` folder of a **Fabric 26.2** instance (Fabric API required). Legacy Launcher / vanilla users: put it in `.minecraft/mods` and run a **Fabric** profile.
